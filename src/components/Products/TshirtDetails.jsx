...
const TshirtDetails = ({ products, onAddToCart }) => {
  const { tshirtID } = useParams();

  return (
    <div>
      {products
        .filter(item => item.id === tshirtID)
        .map(item => ( 
          <div className="row">
          ...
          <div className="col-sm-6 mt-5 d-flex flex-column px-5">
            ...
            <h6 className="fw-bold fs-5 m-0 me-5 price-area py-2 px-3">{item.price.formatted_with_symbol}</h6>
            <button
